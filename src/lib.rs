...
use error::{Error as DobotError, Result as DobotResult};
use message::DobotMessage;
...
impl Dobot {
    ...
    pub async fn set_ptp_cmd(
        ...
        let expected_index = {
            let params = {
                ...
                if params.len() != 4 {
                    return Err(DobotError::DeserializeError(
                        "invalid response length".into(),
                    ));
                }
                params
            };
            let expected_index = u32::from_le_bytes(params.as_slice().try_into().unwrap());
            expected_index
        };
    ...
    pub async fn get_queued_cmd_current_index(&mut self) -> DobotResult<u32> {
        ...
        let params = {
            let params = response_msg.params();
            if params.len() != 4 {
                return Err(DobotError::DeserializeError("invalid response size".into()));
            }
            params
        };
    ...
    async fn send_command(&mut self, request_msg: DobotMessage) -> DobotResult<DobotMessage> {
        // send message
        {
            let mut bytes = request_msg.to_bytes();
            let mut slice: &[u8] = bytes.as_mut_slice();
            tokio::io::copy(&mut slice, &mut self.serial).await?;
        }

        // recive message
        let response_msg = {
            let mut bytes = vec![];
            tokio::io::copy(&mut self.serial, &mut bytes).await?;
            DobotMessage::from_bytes(&bytes)?
        };
